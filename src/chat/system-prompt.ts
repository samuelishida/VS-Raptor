...
export function buildSystemPrompt(options: BuildSystemPromptOptions): string {
  ...
  const toolRef = [
    ...
    `  todoWrite     -- Persist todo list to the current workspace .raptor/todos.json`,
    `  memoryRead    -- Read persistent memory. scope: "project" (default), "global", or "all"`,
    `  memoryWrite   -- Save durable facts. scope: "project" (default workspace-specific) or "global" (explicit user-wide). topic + content + optional replace.`,
    `  spawnAgent    -- Fork a sub-agent with a scoped task. Returns result when done.`,
    ``,
    `## Path rules`,
    `- Use workspace-relative paths for project files, for example src/app.ts or .plans/my-plan/plan.md.`,
    `- Do not use VS Code install paths or other absolute paths outside the workspace; file tools reject them.`,
    ``,
    `## Memory best practices`,
    `- Save project-specific facts (build commands, architecture, conventions) with scope="project"; this is the default`,
    `- Save user-wide preferences (coding style, tool preferences) with scope="global" only when they truly apply across workspaces`,
